...
| --- | --- | --- |
| `apps/server` + `apps/web` | HEIG GitHub Classroom: classes, assignments, student repositories, CI-based grading | classroom.chevallier.io, shared 2 GB Hetzner VM, **production in service** |
...
- The portal's container engine **never** runs on the classroom VM: a root Podman socket, nftables and a 1.5 GB image have no business there (1 vCPU / 2 GB of RAM shared by three services, production database).
